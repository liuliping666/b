...
def parse_args():
    ...
    parser.add_argument("--model", default="davinci-002", type=str)
    parser.add_argument("--prompt_type", default="cot", type=str)
    parser.add_argument("--split", default="validation", type=str)
    parser.add_argument("--num_test_sample", default=3, type=int) # -1 for full data
...
def inference(args):
    ...
    if os.path.exists(data_file):
        ...
        # load data
        if args.data == "hotpot_qa":
            dataset = DatasetLoader.load_dataset(dataset_name=args.data, split=args.split, name="distractor")
        elif args.data == "trivia_qa": # BIG-Bench
            dataset = DatasetLoader.load_dataset(dataset_name=args.data, split=args.split, name="rc.nocontext")
        elif args.data in "ambig_qa":
            dataset = DatasetLoader.load_dataset(dataset_name=args.data, split=args.split) # question only, like BIG-Bench
        else:
    ...
    save_file = f"{save_folder}/{args.split}_{args.prompt_type}_{args.num_test_sample}_seed{args.seed}.jsonl"
